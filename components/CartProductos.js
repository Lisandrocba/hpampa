...
const CartProductos = ({img,name = '', menuName, imgW = 300, sector}) => {
  return (
    <div className={`flex flex-col justify-between items-center w-48 lg:w-1/4 bg-hpampa pb-10 rounded-xl shadow-sm shadow-slate-400 `}>
        <Image priority className="p-6 my-auto object-contain rounded-3xl " src={img} alt='img prod' width={imgW} height={300} priority/> 
        {menuName ? <p className='text-center text-xs lg:text-sm'>{menuName}</p> : null}
